...
met_start = int(sys.argv[1])
met_end = int(sys.argv[2])
...
for obj in bucket.objects.all(): # get all files in all folders? (graphical data, audio, static imgs?)
	...
	if(ext == '.json'):
		print (name)
		timeStamp = int(name.split("_")[4])
		if(timeStamp >= (met_start - _ERROR_ZONE_CONST) and timeStamp <= met_end):
